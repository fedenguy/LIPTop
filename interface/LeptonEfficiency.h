...
{
 public:
  LeptonEfficiency(int era=2012) : era_(era) { }
    ~LeptonEfficiency() {}
	...
	if(era_==2012)
	  {
		...
		if(fabs(eta)<1.479)
		  {
		    if(pt<20)  { eff.first=1.03; eff.second=0.15; }
		    else if(pt<30) { eff.first=0.92; eff.second=0.03; }
		    else if(pt<40) { eff.first=0.957; eff.second=0.006; }
		    else if(pt<50) { eff.first=0.973; eff.second=0.003; }
		    else           {eff.first=0.969; eff.second=0.008; }
		  }
		else
		  {
		    if(pt<20)      { eff.first=1.04; eff.second=0.18; }
		    else if(pt<30) { eff.first=0.95; eff.second=0.03; }
		    else if(pt<40) { eff.first=1.01; eff.second=0.01; }
		    else if(pt<50) { eff.first=0.96; eff.second=0.01; }
		    else           {eff.first=0.98; eff.second=0.02; }
		  }
	      }
